...
基于 [HYM LUMI Walnut 官方资料](https://www.hym-originals-en.com/products/lumi-turntable-walnut)制作的一低音两全频的一体式唱片机概念 CAD。外形与扬声器布局基线为 **v0.9 可调定位合页布局**；当前装配入口为 **v0.4 部分实测弯臂机芯核对版**。扬声器、变压器和功放板已采用用户提供的外廓尺寸；机芯已采用部分实测尺寸，各部件安装接口仍待确认。
...
已按“28高端弯臂机芯全套餐装好直接用 自动回臂”建立 [FreeCAD](cad/lumi-selected-mechanism-fit.FCStd) / [STEP](cad/lumi-selected-mechanism-fit.step) 示意装配，见 [选型说明与重建方法](docs/selected-mechanism.md)。用户已确认到货款除弯臂外与原图一致；采用 Ø280×11 唱盘、85 mm 总高及 R110、约1／4／8点的三个弹簧支点。弹簧自由高15.5、估计压缩0–5 mm，对应电机下探24.5–29.5 mm。底部局部轮廓仍为估算，与现有结构有重叠，安装未放行。
...
| 文件 | 用途 |
|---|---|
| [FreeCAD 装配](cad/lumi-three-driver.FCStd) | 71 个物理零件对象，按结构分组，可隐藏、移动和进一步建模 |
| [STEP 装配](cad/lumi-three-driver.step) | 毫米制实体交换，共 82 个实体；三个扬声器各含 4 个子实体 |
...
[原图、商品链接与安装尺寸](references/fascia/README.md)。音腔顶板孔位按新成形前缘起算，轴承孔局部 Y=166.8 mm；整机坐标未变。
...
机壳外廓为 **450 × 350 × 210.7 mm**，含后凸合页总深 **363.2 mm**（不含未建模紧固件、外接插头和线缆），唱盘直径 **300 mm**，唱臂轴距 **200 mm**，有效长度 **218 mm**。
...
3. 在 FreeCAD 的「宏 → 宏」中选择并执行 `tools/build.FCMacro`。它依次生成 FCStd、STEP、清单、预览（含脚座安装剖视和合页局部视图）和尺寸图；局部视图只裁切临时文档，不修改装配实体。
...
宏每次直接读取 `ac_inlet`、`feet`、`hinges`、`fascia`、`build_model`、`render_views` 和 `dimension_sheet` 七个本地模块的源代码，支持同一 FreeCAD 会话中修改脚本后重跑。生成参数快照保存在 `GeometryDatums.BuildParametersJSON` 中；改动参数却未重建、或模型缺少快照、预检查所列的尺寸基准及安装对象/属性时，验证会先写入 `passed: false` 和具体原因，覆盖旧报告并提示重新生成模型；命令以非零状态退出，验证打开的文档会关闭。报告分别记录模型版本 `revision` 与请求版本 `requested_revision`。
...
2026-09-24 部分实测机芯更新后：基线52项与机芯核对版13项检查通过，17项CAD专项及6项PDF用例通过；当前证据见[工作交接](docs/current-work-handoff.md#验证证据)。

历史记录：2026-09-23 浮动台面后缝调整后，通过 **37 项 CAD 用例和 5 项 PDF 用例**；基线 52 项、机芯核对版 10 项几何检查通过。新增后缝回归覆盖 2 mm 默认值和 7 mm 非默认值，检查保存实体、备料尺寸及邻件无实体重叠。两套 CAD／STEP、预览、备料表和四份 PDF 已同步，台面零件页、孔表、备料页及导览已做渲染检查。验证日志及历史检查的适用范围见[验证证据](docs/current-work-handoff.md#验证证据)。

CAD 回归覆盖参数快照、保存实体尺寸、宏重载、关键部件干涉、音腔泄漏和净容积、倒相管试验长度、脚垫穿板安装及机芯核对版的覆盖保护。T 型铝条的 5 项用例检查真实截面、顶板承托层、邻件间隙和密封边界；错误填实截面、填回台阶、切穿承托层及抬高撞盖均会被检出。合页另有 5 项用例，覆盖保存孔位、盲孔余厚、活动安装件干涉及图纸孔坐标；非默认合页参数回归同时核对保存实体、孔表、材料说明与图纸文字。

图册与导览回归覆盖孔位基准、盲孔深度、真实剖面、对象与图号映射、格栅数量变化及标注遮挡；完整 PDF CLI 用例执行保存 CAD 提取和三册排版，核对页数、位置表及保留图号。测试不会改写原交付文件，但图册与导览用例需要读取当前两套 CAD，因此应先完成重建：
...
v0.9 报告通过全部 **52 项检查**，包括：实体有效性、一低音两全频数量与参数、沿各单元轴线回读的实际外径与总高、生成参数快照一致性、必需模型属性、外廓、唱盘直径、唱臂轴距及有效长度、三音腔独立封闭性、倒相管尺寸与气路、闭盖净空、开盖抽样、关键内部包络、实心安装空间、低音离桌面间隙、变压器本体及固定耳包络尺寸、底板支承与空间避让、功放板含散热片尺寸与避让、STEP 回读实体数、体积与边界一致性，新增脚垫／脚座形状、底板通孔与盲预孔、干涉和法兰支承检查，另含插座孔形、包络、干涉、接线预留与法兰支承检查，以及木板真实法向厚度、整数备料包容关系和矩形格栅节距与间隙。闭盖到机芯包络的最小距离为 **11 mm**。另含 T 型铝条真实截面、邻件干涉、顶板台阶及剩余承托层检查，盖子开合检查纳入铝条。新增合页孔道、盲孔余厚、邻件和活动总成每1度开盖检查；基线共71个物理对象、82个实体。

基线保留盖壳单独每 5° 的开盖检查；两套模型另对盖壳、活动叶、补偿垫片与内压板组成的总成，每 1° 检查 0–70° 共 71 个姿态。两者都是离散抽样，不是连续运动学验证或实物悬停测试。内部检查覆盖喇叭实心圆柱安装包络与机芯、电路板、隔板及外壳，另检查轴承、马达和电子板的指定边界，不代表所有装配细节已放行。音腔已用保存的几何检查独立封闭性与保守净容积，但实际声学容积、扬声器参数匹配、声反馈、隔振性能、驱动与电气功能尚未验证。
