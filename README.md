...
基于 [HYM LUMI Walnut 官方资料](https://www.hym-originals-en.com/products/lumi-turntable-walnut)制作的一低音两全频的一体式唱片机概念 CAD。外形与扬声器布局为 **v0.9 可调定位合页布局**；选定弯臂机芯与结构统一保存在一个主 CAD 文件中，机芯采用 **v0.4 部分实测核对参数**。扬声器、变压器和功放板已采用用户提供的外廓尺寸；机芯安装接口仍待确认。
...
该方式不生成 GUI 颜色与分组可见性设置、预览或尺寸图，也不刷新主验证报告；需要与交付文件一致的展示效果时执行 GUI 宏，再继续验证及图纸流程。

## 验证范围

2026-09-24 已将部分实测机芯与结构合并为单一主 CAD，并默认保存开盖展示姿态。本轮验证证据见[工作交接](docs/current-work-handoff.md#验证证据)。
