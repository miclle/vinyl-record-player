...
基于 [HYM LUMI Walnut 官方资料](https://www.hym-originals-en.com/products/lumi-turntable-walnut)制作的一低音两全频的一体式唱片机概念 CAD。外形与扬声器布局基线为 **v0.8 T 型铝饰条布局**；当前装配入口为 **v0.3 选定弯臂机芯核对版**。扬声器、变压器和功放板已采用用户提供的外廓尺寸；机芯尺寸及各部件安装接口仍待确认。
...
2026-09-23 T 型铝饰条交付完整回归通过 **31 项 CAD 用例和 5 项 PDF 用例**；脚座安装剖视另经独立 GUI 渲染与图像检查，具体批次见[验证证据](docs/current-work-handoff.md#验证证据)。

CAD 回归覆盖参数快照、保存实体尺寸、宏重载、关键部件干涉、音腔泄漏和净容积、倒相管试验长度、脚垫穿板安装及机芯核对版的覆盖保护。T 型铝条新增 5 项用例，检查真实截面、顶板承托层、邻件间隙和密封边界；错误填实截面、填回台阶、切穿承托层及抬高撞盖均会被检出。
...
v0.8 报告通过全部 **45 项检查**，包括：实体有效性、一低音两全频数量与参数、沿各单元轴线回读的实际外径与总高、生成参数快照一致性、必需模型属性、外廓、唱盘直径、唱臂轴距及有效长度、三音腔独立封闭性、倒相管尺寸与气路、闭盖净空、开盖抽样、关键内部包络、实心安装空间、低音离桌面间隙、变压器本体及固定耳包络尺寸、底板支承与空间避让、功放板含散热片尺寸与避让、STEP 回读实体数、体积与边界一致性，新增脚垫／脚座形状、底板通孔与盲预孔、干涉和法兰支承检查，另含插座孔形、包络、干涉、接线预留与法兰支承检查，以及木板真实法向厚度、整数备料包容关系和矩形格栅节距与间隙。闭盖到机芯包络的最小距离为 **11 mm**。另含 T 型铝条真实截面、邻件干涉、顶板台阶及剩余承托层检查，盖子开合检查纳入铝条。
