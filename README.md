...
基于 [HYM LUMI Walnut 官方资料](https://www.hym-originals-en.com/products/lumi-turntable-walnut)制作的一低音两全频的一体式唱片机概念 CAD。外形与扬声器布局基线为 **v0.9 可调定位合页布局**；当前装配入口为 **v0.3 选定弯臂机芯核对版**。扬声器、变压器和功放板已采用用户提供的外廓尺寸；机芯尺寸及各部件安装接口仍待确认。
...
2026-09-23 浮动台面后缝调整后，通过 **37 项 CAD 用例和 5 项 PDF 用例**；基线 52 项、机芯核对版 10 项几何检查通过。新增后缝回归覆盖 2 mm 默认值和 7 mm 非默认值，检查保存实体、备料尺寸及邻件无实体重叠。两套 CAD／STEP、预览、备料表和四份 PDF 已同步，台面零件页、孔表、备料页及导览已做渲染检查。验证日志及历史检查的适用范围见[验证证据](docs/current-work-handoff.md#验证证据)。

CAD 回归覆盖参数快照、保存实体尺寸、宏重载、关键部件干涉、音腔泄漏和净容积、倒相管试验长度、脚垫穿板安装及机芯核对版的覆盖保护。T 型铝条的 5 项用例检查真实截面、顶板承托层、邻件间隙和密封边界；错误填实截面、填回台阶、切穿承托层及抬高撞盖均会被检出。合页另有 4 项用例，覆盖保存孔位、盲孔余厚、活动安装件干涉及图纸孔坐标；非默认合页参数回归同时核对保存实体、孔表、材料说明与图纸文字。
