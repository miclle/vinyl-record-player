...
文档更新：2026-09-22；模型版本：v0.5-integer-woodwork。用户确认扬声器来自 Shockwave SC-2103，原低音箱、卫星箱和倒相管均已遗失，授权按唱机外廓重新设计。该版交付空间方案及可换管试验结构，尚未完成声学调谐或加工放行。

## 布置与尺寸

外廓调整为 450 × 350 × 214.2 mm。坐标原点为左前方桌面，X 向右、Y 向后、Z 向上，单位均为 mm。

![三音腔俯视：上方是后侧，隐藏顶板与前障板以显示内部](../previews/audio-layout.png)

| 项目 | 当前布置 |
|---|---|
| 左右全频单元 | 口端 Ø78、总高 40；中心 X=80 / 370、Z=88，朝前安装于后倾 18° 的障板 |
| 后排低音 | 口端 Ø132、总高 65；中心 X=225、Y=247，安装面 Z=26，朝下；最低点距桌面 23 |
| 中央低音腔 | 净宽 176，X=137–313；从斜面障板内侧延伸至后板内面 Y=338；底面 Z=38、顶面 Z=128 |
| 左右全频腔 | 各净宽 117；斜面障板至 Y=150，Z=38–128；相互及与低音腔不通气，先采用密闭试装 |
| 分隔结构 | 两道 8 mm 全深隔板位于 X=129–137 / 313–321；左右后隔板 Y=150–158；8 mm 声腔顶板 Z=128–136 |
...
- 轴线 X=288、Z=106，沿 Y 方向。外口在后板外面 Y=350，内口 Y=190；从后板内面向腔内伸入 148。
...
修改 `cad/parameters.json` 的 `fullrange`、`woofer`、`acoustic`、`acoustic_divider_x`、`bass_port` 及电子部件参数，按 [README](../README.md#v05-基线重建)先重建并验证基线，再重建机芯核对版。`TweeterLeft` / `TweeterRight` 仅保留为稳定对象 ID，其角色与显示名称已改为全频。更换管长不会自动更新保存的静态实体。
