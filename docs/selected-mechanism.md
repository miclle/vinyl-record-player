...
2026-09-24 用户收到实物，确认除弯形唱臂外，其余结构与原直臂配图一致。原图的 355 mm 总宽据此沿用；唱盘厚度改用实测 11 mm。机芯参数现为 `v0.5-curved-arm-appearance`，资料见[原图与实测记录](../references/selected-mechanism/README.md)。
...
唱臂外观按 `selected-curved-arm-kit.png` 重画：银色管先沿前方伸出，再通过两段相切圆弧向机身右侧偏移 30 mm；前端使用黑色连接套，唱头壳向左偏转 14°，带双槽、三枚圆形顶面垫和侧向指托。模型保持解析直线／圆弧扫掠，便于稳定导出 STEP。管径、15 mm 弯曲半径、弯折位置、连接套和唱头壳局部尺寸都只是照片比例估算，不代表实测尺寸、有效臂长或加工数据。
