...
2026-09-24 用户收到实物，确认除弯形唱臂外，其余结构与原直臂配图一致，并确认唱头指托不会越过机芯基座右侧。原图的 355 mm 总宽据此沿用；唱盘厚度改用实测 11 mm。机芯整体向右移动 12 mm；浮动台面扩展后，355 mm 基座在台面上的左右余量约为 34／33 mm，唱盘中心仍为 X=188 mm。机芯参数现为 `v0.6-centered-mechanism`，资料见[原图与实测记录](../references/mechanism/README.md)。
...
现有弹簧承托面与 10 mm 声腔顶板上表面相差 **12 mm**。弹簧压缩 0／5 mm 时，电机相对承托面的下探为 **24.5／29.5 mm**，在其下方存在顶板的条件下分别缺 **12.5／17.5 mm**，尚未包含动态与装配余量。局部估算体在两个状态均与承载板及声腔顶板重叠；压缩5 mm时还与倒相管重叠。共用板件保留原形，未据此切削或降低整块顶板。
...
浮动台面现为 422 × 330 × 10 mm；到银色 T 型铝饰条后表面为 1 mm、左右各 2 mm、后侧 2 mm，分别由 `deck_front_fascia_clearance`、`deck_side_clearance` 和 `deck_rear_clearance` 控制。v0.6 机芯中心仍为整机 X/Y=(188, 172) mm，主轴孔局部坐标随台面外廓改为 X/Y=(174, 166) mm；支承中心和台面上表面高度未变。三个 Ø16 × 8 mm 支承上下各嵌入顶板／台面 3 mm，沉台及隔振性能仍为安装假设。名义间隙尚未按实物隔振运动验证，见[设计依据](design-basis.md)。
