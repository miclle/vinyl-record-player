...
[整机主模型](../cad/record-player.FCStd) 与[闭盖 STEP](../cad/record-player.step)由结构参数和机芯参数一次生成。主文件默认开盖 70°，结构、当前机芯、历史参考及安装诊断分别分组；后两组默认隐藏。历史通用机芯不进入当前装配或 STEP。无需保留另一份基线 CAD。
