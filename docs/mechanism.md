...
浮动台面现为 422 × 330 × 6 mm；到银色 T 型铝饰条后表面为 1 mm、左右各 2 mm、后侧 2 mm，分别由 `deck_front_fascia_clearance`、`deck_side_clearance` 和 `deck_rear_clearance` 控制。v0.6 机芯中心仍为整机 X/Y=(188, 172) mm，主轴孔局部坐标随台面外廓改为 X/Y=(174, 166) mm；支承位置与台面高度未变。名义间隙尚未按实物隔振运动验证，见[设计依据](design-basis.md)。
