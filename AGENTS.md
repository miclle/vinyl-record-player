...
FreeCAD concept model of a turntable with one downward-facing woofer, two front full-range satellites, a shared transformer, and an amplifier including its heatsink. The selected curved-arm mechanism and fit diagnostics share one main assembly; installation is not released.
...
- `tools/build.FCMacro`: single GUI build entry point; invokes structure/mechanism generation, rendering, and dimension-sheet creation, then saves a 70-degree open cover pose. `tools/assembly_pose.py` stores the closed placements; `tools/toggle-cover.FCMacro` toggles the viewing pose without saving.
...
- `tools/validate_model.py`: main assembly geometry and closed STEP validation; `tests/test_regressions.py`: regression tests; `tests/test_ac_inlet.py`: rear inlet opening and wiring-space checks.
- `tools/mechanism_study.py`: selected-mechanism geometry, fit diagnostics/report, and GUI previews inside the main build; `tests/test_mechanism_study.py`: fit-study regression tests.
- `cad/parameters.json`: cabinet, drivers, transformer, amplifier, AC inlet, feet, T-fascia and hinge installation inputs; `parts.csv`, `wood-cut-list.csv`, and `validation.json` describe the current main assembly.
...
- `cad/selected-mechanism.json`: mechanism assumptions. `lumi-selected-mechanism-fit.FCStd` is the only main CAD; its `.step` is the closed physical assembly and `mechanism-fit-report.json` records its hash. The old `lumi-three-driver.*` and `lumi-dual-driver.*` files are available in Git history. The hidden `Mechanism` group retains 16 historical reference objects for existing drawing appendices; exclude `IsReference` and `IsDiagnostic` objects from physical exports, parts lists, collisions and acoustic occupancy.
...
After either JSON changes, rebuild the main assembly once. Both parameter snapshots are saved in the same document. The fit report records `model_sha256` after the final GUI save. FCStd defaults to a 70-degree open cover; STEP, parts dimensions, validation and drawing extraction use the closed datum. Read-only consumers must restore the cover assembly via `assembly_pose.set_cover_angle(doc, parameters, 0)` on a private copy. Renderers move all `hinges.moving_names()` together and must preserve the saved closed placements. Viewing macros must not silently save user documents or invalidate report hashes.
...
`foot_height` must equal `feet.rubber_height + feet.flange_thickness` for the supported flush-contact installation. Changing foot dimensions does not automatically translate the other absolute Z parameters; update the linked heights described in `references/feet/README.md` and rebuild the main CAD and its drawings. Spacers or a different mount orientation require a geometry change, not just a new `foot_height` value. Chamber closure includes the saved foot and mount solids in ideal contact; it is not proof of real thread or panel sealing.
...
Hinge symmetry and inlet clearance depend on three independent inputs: `hinges.center_x`, `ac_inlet.center_z`, and `power_transformer.center_y`. Changing one does not reposition the others. Rebuild the main assembly and the A-H02/A-H03 hole sheets after changing this layout; retain the wiring reservation and check transformer clearance. Do not move only a `HingePin` in the GUI. In the rear view, object suffix 0 at X=100 appears on the right.

The v0.4 selected-mechanism study uses partial user measurements: platter Ø280×11, total height85, platform108×128×5, 40mm above the platform and 40mm below the spring seat. Treat seat/platform-bottom coplanarity as an interpretation assumption. Three spring centers are approximately R110 at 1/4/8 o'clock (top view, 12 rear); free height15.5 and estimated compression0–5 imply motor depth24.5–29.5 below the support plane. `measurement_datums` owns this height chain. Do not confuse arm total length250 with effective length; its parked longitudinal interpretation remains assumed. `appearance_assumptions` contains unmeasured local motor/linkage outlines and positions. `UnderbodyReservation` is incomplete diagnostic geometry, excluded from STEP; absence of overlap cannot release fit. `KitBase` includes three illustrative spring sleeves, covered by existing K01/C-P05 references. Preserve baseline wood geometry until interfaces are verified; synchronize both spring endpoint checks, study previews, drawings and guide after study changes. Both endpoint closed-cover and sampled-sweep collisions must fail geometry validation even when the displayed state is clear. Generate spring clock-position notes from configuration in both C-00 and C-P05, and test nondefault positions against saved solids and PDF text.
